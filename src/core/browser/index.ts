...
const isConnectionError = (error: unknown): boolean =>
  error instanceof Error && error.message.includes(CONNECTION_ERROR);
...
export const sendMessage = <M = unknown, R = unknown>(message: M): Promise<R> =>
  sendMessageWithRetry<M, R>(message, 1);
