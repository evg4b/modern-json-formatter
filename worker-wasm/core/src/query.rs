...
mod faq_basic_filters {
    ...
    fn expect(input: &str, query: &str, outputs: &[&str]) -> Node {
        ...
        Node::tuple(outputs.iter().map(|o| node(o)).collect())
    }
...
mod faq_types_and_values {
    ...
    fn array_construction_user_projects() {
        ...
        assert_eq!(result, Node::tuple(vec![node("[\"stedolan\", \"jq\", \"wikiflow\"]")]));
    }

    #[test]
    fn array_construction_double_elements() {
        let result = query_json("[1, 2, 3]", "[ .[] | . * 2]").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[2, 4, 6]")]));
    }
    ...
    fn object_construction_multiple_outputs() {
        ...
        assert_eq!(result, Node::tuple(vec![
    ...
    fn object_construction_dynamic_key() {
        ...
        assert_eq!(result, Node::tuple(vec![node("{\"stedolan\": [\"JQ Primer\", \"More JQ\"]}")]));
    }

    #[test]
    fn recursive_descent() {
        let result = query_json("[[{\"a\":1}]]", ".. | .a?").unwrap();
        assert_eq!(result, Node::tuple(vec![node("1")]));
    }
}
...
mod faq_assignment {
    ...
    fn update_assignment_range_first_value() {
        let result = query_json("null", "(.a, .b) |= range(3)").unwrap();
        assert_eq!(result, Node::tuple(vec![node("{\"a\":0,\"b\":0}")]));
    }
}
...
mod faq_advanced_features {
    ...
    fn reduce_object_destructuring() {
        ...
        assert_eq!(result, Node::tuple(vec![node("{\"x\":\"abc\",\"y\":[1,2,3]}")]));
    }
    ...
    fn generators_custom_while() {
        ...
        assert_eq!(result, Node::tuple(vec![node("[1,2,4,8,16,32,64]")]));
    }
}
...
mod faq_builtin_operators_and_functions {
    ...
    fn add_number() {
        ...
        assert_eq!(result, Node::tuple(vec![node("8")]));
    }

    #[test]
    fn add_arrays() {
        let result = query_json("{\"a\": [1,2], \"b\": [3,4]}", ".a + .b").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[1,2,3,4]")]));
    }
    ...
    fn add_objects_merge() {
        ...
        assert_eq!(result, Node::tuple(vec![node("{\"a\": 42, \"b\": 2, \"c\": 3}")]));
    }
    ...
    fn subtract_array_elements() {
        ...
        assert_eq!(result, Node::tuple(vec![node("[\"json\"]")]));
    }
    ...
    fn multiply_divide() {
        ...
        assert_eq!(result, Node::tuple(vec![node("6")]));
    }

    #[test]
    fn divide_string() {
        let result = query_json("\"a, b,c,d, e\"", ". / \", \"").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[\"a\",\"b,c,d\",\"e\"]")]));
    }

    #[test]
    fn multiply_objects_recursive_merge() {
        let result = query_json("null", "{\"k\": {\"a\": 1, \"b\": 2}} * {\"k\": {\"a\": 0,\"c\": 3}}").unwrap();
        assert_eq!(result, Node::tuple(vec![node("{\"k\": {\"a\": 0, \"b\": 2, \"c\": 3}}")]));
    }
    ...
    fn divide_by_zero_suppressed() {
        ...
        assert_eq!(result, Node::tuple(vec![node("1"), node("-1")]));
    }

    #[test]
    fn abs_array() {
        let result = query_json("[-10, -1.1, -1e-1]", "map(abs)").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[10,1.1,1e-1]")]));
    }
    ...
    fn utf8bytelength() {
        ...
        assert_eq!(result, Node::tuple(vec![node("2")]));
    }

    #[test]
    fn keys_object_sorted() {
        let result = query_json("{\"abc\": 1, \"abcd\": 2, \"Foo\": 3}", "keys").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[\"Foo\", \"abc\", \"abcd\"]")]));
    }
    ...
    fn has_key_object() {
        ...
        assert_eq!(result, Node::tuple(vec![node("[true, false]")]));
    }

    #[test]
    fn has_key_array_index() {
        let result = query_json("[[0,1], [\"a\",\"b\",\"c\"]]", "map(has(2))").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[false, true]")]));
    }

    #[test]
    fn in_object() {
        let result = query_json("[\"foo\", \"bar\"]", ".[] | in({\"foo\": 42})").unwrap();
        assert_eq!(result, Node::tuple(vec![node("true"), node("false")]));
    }
    ...
    fn pick_array_indices() {
        ...
        assert_eq!(result, Node::tuple(vec![node("[1,null,3]")]));
    }

    #[test]
    fn path_expression() {
        let result = query_json("null", "path(.a[0].b)").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[\"a\",0,\"b\"]")]));
    }

    #[test]
    fn path_recursive() {
        let result = query_json("{\"a\":[{\"b\":1}]}", "[path(..)]").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[[],[\"a\"],[\"a\",0],[\"a\",0,\"b\"]]")]));
    }
    ...
    fn del_field() {
        ...
        assert_eq!(result, Node::tuple(vec![node("{\"bar\": 9001, \"baz\": 42}")]));
    }
    ...
    fn del_array_indices() {
        ...
        assert_eq!(result, Node::tuple(vec![node("[\"foo\"]")]));
    }
    ...
    fn setpath_null_input() {
        ...
        assert_eq!(result, Node::tuple(vec![node("{\"a\": {\"b\": 1}}")]));
    }
    ...
    fn flatten_empty_nested() {
        ...
        assert_eq!(result, Node::tuple(vec![node("[]")]));
    }

    #[test]
    fn flatten_objects_not_flattened() {
        let result = query_json("[{\"foo\": \"bar\"}, [{\"foo\": \"baz\"}]]", "flatten").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[{\"foo\": \"bar\"}, {\"foo\": \"baz\"}]")]));
    }

    #[test]
    fn range_from_to() {
        let result = query_json("null", "range(2; 4)").unwrap();
        assert_eq!(result, Node::tuple(vec![node("2"), node("3")]));
    }

    #[test]
    fn range_collect() {
        let result = query_json("null", "[range(2; 4)]").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[2,3]")]));
    }

    #[test]
    fn range_upto() {
        let result = query_json("null", "[range(4)]").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[0,1,2,3]")]));
    }

    #[test]
    fn range_with_step() {
        let result = query_json("null", "[range(0; 10; 3)]").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[0,3,6,9]")]));
    }
    ...
    fn range_negative_step_down() {
        ...
        assert_eq!(result, Node::tuple(vec![node("[0,-1,-2,-3,-4]")]));
    }

    #[test]
    fn floor_pi() {
        let result = query_json("3.14159", "floor").unwrap();
        assert_eq!(result, Node::tuple(vec![node("3")]));
    }
    ...
    fn tostring_various() {
        ...
        assert_eq!(result, Node::tuple(vec![node("\"1\""), node("\"1\""), node("\"[1]\"")]));
    }

    #[test]
    fn type_of_values() {
        let result = query_json("[0, false, [], {}, null, \"hello\"]", "map(type)").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[\"number\", \"boolean\", \"array\", \"object\", \"null\", \"string\"]")]));
    }
    ...
    fn infinite_nan_type() {
        ...
        assert_eq!(result, Node::tuple(vec![node("\"number\""), node("\"number\"")]));
    }

    #[test]
    fn sort_array() {
        let result = query_json("[8,3,null,6]", "sort").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[null,3,6,8]")]));
    }

    #[test]
    fn sort_by_field() {
        let result = query_json("[{\"foo\":4, \"bar\":10}, {\"foo\":3, \"bar\":10}, {\"foo\":2, \"bar\":1}]", "sort_by(.foo)").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[{\"foo\":2, \"bar\":1}, {\"foo\":3, \"bar\":10}, {\"foo\":4, \"bar\":10}]")]));
    }

    #[test]
    fn sort_by_two_fields() {
        let result = query_json("[{\"foo\":4, \"bar\":10}, {\"foo\":3, \"bar\":20}, {\"foo\":2, \"bar\":1}, {\"foo\":3, \"bar\":10}]", "sort_by(.foo, .bar)").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[{\"foo\":2, \"bar\":1}, {\"foo\":3, \"bar\":10}, {\"foo\":3, \"bar\":20}, {\"foo\":4, \"bar\":10}]")]));
    }

    #[test]
    fn group_by_field() {
        let result = query_json("[{\"foo\":1, \"bar\":10}, {\"foo\":3, \"bar\":100}, {\"foo\":1, \"bar\":1}]", "group_by(.foo)").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[[{\"foo\":1, \"bar\":10}, {\"foo\":1, \"bar\":1}], [{\"foo\":3, \"bar\":100}]]")]));
    }
    ...
    fn max_by_field() {
        ...
        assert_eq!(result, Node::tuple(vec![node("{\"foo\":2, \"bar\":3}")]));
    }

    #[test]
    fn unique_numbers() {
        let result = query_json("[1,2,5,3,5,3,1,3]", "unique").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[1,2,3,5]")]));
    }

    #[test]
    fn unique_by_field() {
        let result = query_json("[{\"foo\": 1, \"bar\": 2}, {\"foo\": 1, \"bar\": 3}, {\"foo\": 4, \"bar\": 5}]", "unique_by(.foo)").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[{\"foo\": 1, \"bar\": 2}, {\"foo\": 4, \"bar\": 5}]")]));
    }

    #[test]
    fn unique_by_length() {
        let result = query_json("[\"chunky\", \"bacon\", \"kitten\", \"cicada\", \"asparagus\"]", "unique_by(length)").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[\"bacon\", \"chunky\", \"asparagus\"]")]));
    }

    #[test]
    fn reverse_array() {
        let result = query_json("[1,2,3,4]", "reverse").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[4,3,2,1]")]));
    }
    ...
    fn rindex_string() {
        ...
        assert_eq!(result, Node::tuple(vec![node("12")]));
    }

    #[test]
    fn rindex_array_element() {
        let result = query_json("[0,1,2,1,3,1,4]", "rindex(1)").unwrap();
        assert_eq!(result, Node::tuple(vec![node("5")]));
    }
    ...
    fn startswith() {
        ...
        assert_eq!(result, Node::tuple(vec![node("[false, true, false, true, false]")]));
    }
    ...
    fn ltrimstr() {
        ...
        assert_eq!(result, Node::tuple(vec![node("[\"fo\",\"\",\"barfoo\",\"bar\",\"afoo\"]")]));
    }

    #[test]
    fn rtrimstr() {
        let result = query_json("[\"fo\", \"foo\", \"barfoo\", \"foobar\", \"foob\"]", "[.[]|rtrimstr(\"foo\")]").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[\"fo\",\"\",\"bar\",\"foobar\",\"foob\"]")]));
    }

    #[test]
    fn explode() {
        let result = query_json("\"foobar\"", "explode").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[102,111,111,98,97,114]")]));
    }
    ...
    fn join_mixed_types() {
        ...
        assert_eq!(result, Node::tuple(vec![node("\"a 1 2.3 true false\"")]));
    }

    #[test]
    fn ascii_upcase() {
        let result = query_json("\"useful but not for é\"", "ascii_upcase").unwrap();
        assert_eq!(result, Node::tuple(vec![node("\"USEFUL BUT NOT FOR é\"")]));
    }
    ...
    fn until_factorial() {
        ...
        assert_eq!(result, Node::tuple(vec![node("24")]));
    }
    ...
    fn string_interpolation() {
        ...
        assert_eq!(result, Node::tuple(vec![node("\"The input was 42, which is one less than 43\"")]));
    }

    #[test]
    fn tostring_various_types() {
        let result = query_json("[1, \"foo\", [\"foo\"]]", "[.[]|tostring]").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[\"1\",\"foo\",\"[\\\"foo\\\"]\"]")]));
    }
...
mod faq_conditionals_and_comparisons {
    ...
    fn if_elif_else() {
        ...
        assert_eq!(result, Node::tuple(vec![node("\"many\"")]));
    }
    ...
    fn alternative_empty() {
        ...
        assert_eq!(result, Node::tuple(vec![node("42")]));
    }

    #[test]
    fn alternative_existing_value() {
        let result = query_json("{\"foo\": 19}", ".foo // 42").unwrap();
        assert_eq!(result, Node::tuple(vec![node("19")]));
    }
    ...
    fn optional_operator_tonumber() {
        ...
        assert_eq!(result, Node::tuple(vec![node("[1, 3, 4]")]));
    }
}
...
mod faq_regular_expressions {
    ...
    fn test_flags_ix() {
        ...
        assert_eq!(result, Node::tuple(vec![node("true"), node("true")]));
    }
    ...
    fn match_simple() {
        ...
        assert_eq!(result, Node::tuple(vec![node("{\"offset\": 0, \"length\": 3, \"string\": \"foo\", \"captures\": []}")]));
    }
    ...
    fn capture_named_groups() {
        ...
        assert_eq!(result, Node::tuple(vec![node("{ \"a\": \"xyzzy\", \"n\": \"14\" }")]));
    }
    ...
    fn scan_occurrences() {
        ...
        assert_eq!(result, Node::tuple(vec![node("\"c\""), node("\"c\"")]));
    }

    #[test]
    fn split_regex() {
        let result = query_json("\"ab,cd, ef\"", "split(\", *\"; null)").unwrap();
        assert_eq!(result, Node::tuple(vec![node("[\"ab\",\"cd\",\"ef\"]")]));
    }
...
mod tests {
    ...
    fn query_returns_float_literal() {
        ...
        assert_eq!(result, Node::tuple(vec![node("1.5")]));
    }
    ...
    fn query_returns_array_value() {
        ...
        assert_eq!(
            result,
            Node::tuple(vec![NodeJsonFactory.array(vec![node("1"), node("2")])]),
        );
    }
    ...
    fn query_returns_multiple_results() {
        ...
        assert_eq!(
            result,
            Node::tuple(vec![node("1"), node("2"), node("3")]),
        );
    }
